...
class SimConfig:
    ...
    delta_t: int = 300              # Macro-control decision interval / block-mean window [s]
    ...
    # =========================================================================
    Q_b: float = 500.0        # Nominal capacity of the battery pack [kWh]
    ...
    N_d: int = 6         # Number of discrete load levels (Power demand Grid)
    N_fc: int = 12         # Number of fuel cell modules on board
    delta_n: int = 1       # Number of fuel cell modules in a pack

    N_s: int = 25       # Grid resolution for SoC dimension (SoC Grid)
    N_b: int = 15        # Grid resolution for P_batt dimension (P_batt grid)
    N_f: int = 20       # Grid resolution for previous P_fc dimension

    use_smart_grid: bool = True
    delta_P: float = 200.0
    verbose: bool = True
    grid_method: str = 'geometric' # Options: 'geometric', 'linear', 'quantile', 'frequency', 'mass_variance'

    # Derived fields (populated automatically in __post_init__)
    C_rep: float = field(init=False)
    Q_eol: float = field(init=False)
    ...
    def __post_init__(self):
        ...
        object.__setattr__(self, 'C_rep', self.Q_b * self.c_bat_kwh)
        object.__setattr__(self, 'Q_eol', 2.0 * self.n_cycles_rated * self.dod_rated * self.Q_b)
        ...
        # --- GRID GENERATION ---
        object.__setattr__(self, 'n_vals', np.arange(0, self.N_fc+1, self.delta_n, dtype=np.int32))
        max_fc_power = self.N_fc * self.p_max

        if self.use_smart_grid:
            # 1. Congruent Battery Power Grid
            pb_grid = np.arange(self.pb_min, self.pb_max + self.delta_P, self.delta_P)
            object.__setattr__(self, 'pb_vals', pb_grid)
            object.__setattr__(self, 'N_b', len(pb_grid))
            
            # 2. Congruent FC Power Grid
            pfc_grid = np.arange(0.0, max_fc_power + self.delta_P, self.delta_P)
            object.__setattr__(self, 'pfc_vals', pfc_grid)
            object.__setattr__(self, 'N_f', len(pfc_grid))
            
            # 3. Congruent SoC Grid (Lattice Phasing: Anchored to soc_initial)
            dSoC = (self.delta_P * self.delta_t) / (self.Q_b * 3600.0)
            ...
            object.__setattr__(self, 'N_s', len(soc_grid))
            ...
            # Legacy Manual Grids
            object.__setattr__(self, 'soc_vals', np.linspace(self.soc_min, self.soc_max, self.N_s))
            object.__setattr__(self, 'pb_vals', np.linspace(self.pb_min, self.pb_max, self.N_b))
            object.__setattr__(self, 'pfc_vals', np.linspace(0.0, max_fc_power, self.N_f))
            
        # Fire the diagnostic tracker
        if self.verbose:
            self._print_complexity_diagnostics(self.N_d)
        ...
        if not (self.delta_t / self.dt).is_integer():
            raise ValueError("Macro time step delta_t must be a perfect multiple of dt.")
    ...
    def _print_complexity_diagnostics(self, N_d):
            ...
            T_steps = int(seconds_in_day / self.delta_t)
            
            # O(S) = N_d * N_fc * N_s * N_f
            S_nodes = N_d * N_n * self.N_s * self.N_f
            
            # O(A) = N_fc * N_bat
            A_nodes = N_n * self.N_b
            
            # O(Comp) = T * |S| * (|N_d| + |A|)
            transitions = T_steps * S_nodes * (N_d + A_nodes)
            ...
            print("="*55)
            if self.use_smart_grid:
                print(f" -> delta_P Step Size       : {self.delta_P} kW")
            if self.delta_n != 1:
                print(f" -> Module packs size  : {self.delta_n} modules")

            print(f" -> Grid Dimensions    : N_d={N_d}, N_fc={N_n}, N_s={self.N_s}, N_fc={self.N_f}, N_bat={self.N_b}")
